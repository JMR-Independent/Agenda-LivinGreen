        ...
        function closeJobDropdown() {
            ...
            if (sheet) { sheet.style.display = 'none'; sheet.classList.remove('sheet-open'); }
        ...
        async function extractDataWithGeminiVision(imagesData) {
            try {
                ...
                if (!key) { console.log('⚠️ Gemini key not found (rize_gemini_key)'); return null; }
                console.log('🤖 Attempting Gemini Vision OCR...');

                // Use cached model or discover one
                let model = localStorage.getItem('rize_gemini_model');
                if (!model) {
                    const candidates = ['gemini-2.5-flash-preview-05-20','gemini-2.5-flash','gemini-2.0-flash-exp','gemini-1.5-flash'];
                ...
                if (!model) { console.log('⚠️ No Gemini model available'); return null; }

                const prompt = `Analiza esta imagen de conversación (WhatsApp, Messenger, SMS, texto) y extrae los datos de la cita de limpieza.

CAMPOS:
- name: Nombre completo del cliente (busca en el encabezado del contacto, en el texto, firma, presentación)
- time: Hora en formato 24h HH:MM (convierte: "2pm"→"14:00", "10am"→"10:00", "3 de la tarde"→"15:00", "noon"→"12:00")
- day: Día de la semana en español (lunes/martes/miércoles/jueves/viernes/sábado/domingo) O fecha MM/DD si aparece
- city: Ciudad donde se realiza el trabajo
- address: Dirección completa (incluye formatos Utah como "123 N 400 W" o "5 N 100 E Apt 2")
- job: Tipo de servicio (Standard Clean, Deep Clean, Move In, Move Out, AirBnB, Office Clean, Carpet Cleaning, etc.)
- price: Solo el número del precio (sin símbolo $)

REGLAS: Lee TODO el texto. Si un dato no aparece deja el campo vacío. NO inventes datos.
...
{"name":"","time":"","day":"","city":"","address":"","job":"","price":""}`;
                ...
                for (let i = 0; i < imagesData.length; i++) {
                    console.log(`📷 Gemini Vision: image ${i+1}/${imagesData.length}`);
                    const loadingEl = document.getElementById('loading');
                    if (loadingEl) {
                        const pt = loadingEl.querySelector('p');
                        if (pt) pt.textContent = `Analizando imagen ${i+1}/${imagesData.length} con IA...`;
                    }

                    const body = {
                        contents: [{ role: 'user', parts: [
                            { inlineData: { mimeType: 'image/jpeg', data: imagesData[i].base64 } },
                            { text: prompt }
                        ]}],
                    ...
                    if (!res.ok) { console.warn(`⚠️ Gemini Vision error image ${i+1}: ${res.status}`); continue; }

                    const data = await res.json();
                    const text = data.candidates?.[0]?.content?.parts?.map(p => p.text || '').join('') || '';
                    if (!text) { console.warn(`⚠️ No text from Gemini image ${i+1}`); continue; }
                    ...
                    if (jsonMatch) {
                        try {
                            ...
                            console.log(`✅ Gemini Vision image ${i+1}:`, parsed);
                            extractedDataArray.push(parsed);
                        } catch (e) { console.warn(`⚠️ JSON parse failed image ${i+1}`, e); }
                    }
                }

                if (!extractedDataArray.length) return null;
                return combineExtractedData(extractedDataArray);
            } catch (e) {
        ...
        async function extractDataFromMultipleImages(imagesData) {
            console.log(`🔄 Processing ${imagesData.length} images for data extraction...`);

            try {
                // Try Gemini Vision first (uses existing rize_gemini_key, no backend needed)
                console.log('🤖 Attempting Gemini Vision OCR...');
                ...
                if (geminiResult && Object.values(geminiResult).some(v => v && String(v).trim())) {
                    console.log('✅ Gemini Vision extraction successful:', geminiResult);
                    return geminiResult;
                }
                console.log('⚠️ Gemini Vision failed, trying OpenAI...');

                // Try OpenAI GPT-4 Vision as second option
                console.log('🤖 Attempting OpenAI GPT-4 Vision OCR...');
                const openaiResult = await extractDataWithOpenAI(imagesData);
                if (openaiResult && Object.values(openaiResult).some(v => v && String(v).trim())) {
                    console.log('✅ OpenAI extraction successful:', openaiResult);
                    return openaiResult;
                }
                console.log('⚠️ OpenAI extraction failed, trying next method...');
                ...
                if (visionResult && Object.values(visionResult).some(v => v && String(v).trim())) {
                    console.log('✅ Google Vision extraction successful:', visionResult);
                    return visionResult;
                }
                console.log('⚠️ Google Vision extraction failed, trying next method...');

                // Try Tesseract.js as third option (free, runs in browser)
                console.log('📝 Attempting Tesseract.js OCR (free, browser-based)...');
                const tesseractResult = await extractDataWithTesseract(imagesData);
                if (tesseractResult && Object.keys(tesseractResult).length > 0) {
                    console.log('✅ Tesseract extraction successful:', tesseractResult);
                    return tesseractResult;
                }
                console.log('⚠️ Tesseract extraction failed, trying manual extraction...');
  ...
  "price": "precio en número"
        ...
        function fillForm(data) {
            ...
            if (data.job) {
                ...
                // Try to match options — case-insensitive, bidirectional substring
                checkboxes.forEach(checkbox => {
                    const cbVal = checkbox.value.toLowerCase();
                    if (jobText.includes(cbVal) || cbVal.includes(jobText) ||
                        jobText.split(/[\s,\/]+/).some(word => word.length > 3 && cbVal.includes(word))) {
                ...
                if (matchedAny) {
                    ...
                    console.log('✅ Set job checkboxes from:', data.job);
                } else {
                    console.log('⚠️ No job matches found for:', data.job);
                }
            }
        ...
        async function handleFormSubmit(e) {
            ...
            if (selectedDate === today && appointmentsToday >= MAX_APPOINTMENTS_PER_DAY) {
                showError('Ya se alcanzó el límite de 3 citas para hoy.');
                return;
            }
            ...
            if (selectedDate !== today) {
                ...
                console.log(`📊 Appointments for ${selectedDate}:`, appointmentsForSelectedDate.length);

                if (appointmentsForSelectedDate.length >= MAX_APPOINTMENTS_PER_DAY) {
                    showError(`Ya se alcanzó el límite de 3 citas para el ${new Date(selectedDate).toLocaleDateString('es-ES')}.`);
                    return;
                }
            }
        ...
        function updateAppointmentCounter() {
            ...
            if (appointmentsToday >= MAX_APPOINTMENTS_PER_DAY) {
                submitBtn.disabled = true;
                submitBtn.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:0.5;">Límite diario alcanzado</div>';
            } else {
                submitBtn.disabled = false;
                restoreSubmitBtn();
            }
            ...
            if (landingView) {
                ...
                setTimeout(() => {
                    ...
                    if (viewName === 'schedule') {
                        ...
                        // Reset upload bar fade (no scale — avoids jump)
                        const uploadArea = document.getElementById('upload-area');
                        if (uploadArea) {
                    ...
                    // Load view-specific data (only if not already loaded)
                ...
                }, 150); // Match landing-exit duration
            }
        ...
        const DEFAULT_SERVICES_CONFIG = [
            { id: 'alfombras',  name: 'Alfombras',  basePrice: 30,  priceRange: '$30 cada una' },
            { id: 'sillones',   name: 'Sillones',   basePrice: 85,  priceRange: '$85 cada uno' },
            { id: 'escaleras',  name: 'Escaleras',  basePrice: 50,  priceRange: '$45 - $55' },
            { id: 'cuartos',    name: 'Cuartos',    basePrice: 30,  priceRange: '$30 cada uno' },
            { id: 'sala',       name: 'Sala',       basePrice: 45,  priceRange: '$30 - $60' },
            { id: 'pasillo',    name: 'Pasillo',    basePrice: 20,  priceRange: '$10 - $30' },
            { id: 'sillas',     name: 'Sillas',     basePrice: 10,  priceRange: '$10 cada una' },
            { id: 'colchon',    name: 'Colchón',    basePrice: 65,  priceRange: '$65 cada uno' },
            { id: 'auto',       name: 'Auto',       basePrice: 100, priceRange: 'Desde $100' },
        ];
        let servicesConfig = [];
        ...
        function initServicesConfig() {
            try {
                ...
                servicesConfig = (Array.isArray(parsed) && parsed.length > 0) ? parsed : [...DEFAULT_SERVICES_CONFIG];
            } catch(e) {
                servicesConfig = [...DEFAULT_SERVICES_CONFIG];
            }
            renderJobDropdown();
        ...
        function renderJobDropdown() {
            const jobSelect = document.getElementById('job');
            if (jobSelect) {
                jobSelect.innerHTML = servicesConfig.map(svc =>
        ...
        function renderServiceTypeSelect() {
            ...
            select.innerHTML = '<option value="">Seleccionar servicio</option>' +
                servicesConfig.map(svc =>
                    `<option value="${svc.id}">${svc.name} — ${svc.priceRange}</option>`
                ).join('');
        }
        ...
        function buildAISystemPrompt() {
            ...
            return `Eres el asistente inteligente de ${bc.name}, empresa de limpieza en Utah, USA. Eres un AGENTE COMPLETO con conocimiento general ilimitado y acceso total a la app. Respondes en español.
                ...
                <div class="mobile-invoice-container" style="max-width: 400px !important; width: 400px !important; margin: 0 auto !important; background: white !important; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif !important; border-radius: 12px !important; overflow: hidden !important; box-shadow: 0 2px 20px rgba(0,0,0,0.1) !important; position: relative !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important;">
                        ...
                        <div style="display: flex; justify-content: center; align-items: center; margin-bottom: 20px;">
                            <div style="background: #28a745; color: white; padding: 8px 16px; border-radius: 20px; display: flex; align-items: center; gap: 8px; font-weight: 500;">
                                <div style="width: 20px; height: 20px; background: white; border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                                    <div style="color: #28a745; font-size: 14px; font-weight: bold;">✓</div>
                                </div>
                                <span>Paid</span>
                            </div>
                        </div>
                    ...
                    <div style="padding: 20px;">
                        ...
                        <div style="text-align: center; margin: 12px 0 8px 0;">
                            <img src="${getBC().logo || getBC().logoDefault}" alt="${getBC().name} Logo" class="biz-logo-dynamic" style="width: 120px; height: auto; display: block; margin: 0 auto; transform: scale(1.2); object-fit: contain; max-height: 60px; object-position: center;">
                        </div>
...
END:VCARD`;
        ...
        function checkPendingNotifications() {
            ...
            try {
                const dv = JSON.parse(localStorage.getItem('dayEvents') || '[]');
                if (Array.isArray(dv)) dv.forEach(e => { if (!apts.find(a => a.id === e.id)) apts.push(e); });
            } catch {}

            apts.forEach(apt => {
                const dateStr = apt.date || apt.fecha || '';
                if (!dateStr) return;
                const timeStr = apt.time || apt.hora || '09:00';
                const aptTime = new Date(dateStr + 'T' + timeStr).getTime();
                if (isNaN(aptTime) || aptTime < now) return; // already past

                const reminderTime = aptTime - 24 * 60 * 60 * 1000;
                const notifKey = 'notif_apt_' + (apt.id || dateStr);

                if (localStorage.getItem(notifKey)) return; // already sent

                const clientName = apt.clientName || apt.cliente || apt.name || 'Cliente';
                const service = apt.service || apt.servicio || apt.job || 'Servicio';
                const city = apt.city || apt.ciudad || '';
                const title = '🗓 Cita mañana — ' + clientName;
                const body = service + (timeStr ? ' a las ' + timeStr : '') + (city ? ' · ' + city : '');

                if (now >= reminderTime) {
                    // Window passed — show immediately
                    _showNotification(title, body, notifKey);
                    localStorage.setItem(notifKey, '1');
                } else {
                    const delay = reminderTime - now;
                    // Schedule via SW if delay < 24h (SW might die after long delays, but handles most cases)
                    _scheduleNotification(title, body, notifKey, delay);
                }
            });

            // ── 2. Facebook promotion reminder — every 2 days ──────────
            const lastFb = parseInt(localStorage.getItem('notif_fb_last') || '0');
            const twoDays = 2 * 24 * 60 * 60 * 1000;
            if (now - lastFb >= twoDays) {
                _showNotification('🗓 Revisar Facebook', 'Han pasado 2 días — revisa y actualiza tus promociones en Facebook', 'fb-promo');
                localStorage.setItem('notif_fb_last', now.toString());
            }
        }
        ...
        function _showNotification(title, body, tag) {
            ...
            if (_swReg) {
                navigator.serviceWorker.ready.then(reg => {
                    reg.active && reg.active.postMessage({ type: 'SHOW_NOTIF', title, body, tag });
                });
            } else {
                try { new Notification(title, { body, icon: './images/agenda-logo.jpg', tag }); } catch {}
            }
        }

        function _scheduleNotification(title, body, tag, delay) {
            if (Notification.permission !== 'granted') return;
            // Schedule via SW message (handles page-close scenarios better)
            navigator.serviceWorker.ready.then(reg => {
                reg.active && reg.active.postMessage({ type: 'SCHEDULE_NOTIF', title, body, tag, delay });
            }).catch(() => {
                // Fallback: setTimeout in main thread (only works if page stays open)
                setTimeout(() => _showNotification(title, body, tag), delay);
            });
        }
        ...
        function updateNotifToggleUI() {
            ...
            status.textContent = granted ? 'Recibirás recordatorios 24h antes de cada cita y cada 2 días para revisar Facebook.' : 'Toca el botón para activar.';
        }
