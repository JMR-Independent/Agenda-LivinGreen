...
function _updateSubtitle(text) {
  // subtitle stays fixed as "LivinGreen"
}
...
function _lg_fullSync(showFeedback=false){
  try{
    ...
    if(subEl) subEl.textContent='LivinGreen';
...
(function setupFocus(){
  ...
  // Focus only when user taps input directly
...
function buildSystemPrompt() {
  ...
  const mtdApts = allApts.filter(a=>{const[y,m]=(a.date||a.fecha||'').split('-').map(Number);return y===curY&&m===curM;});
  const mtdRevenue = mtdApts.reduce((s,a)=>s+(parseFloat(a.price)||0),0);
...
Horario típico: Lunes–Sábado 8am–6pm
...
MES ACTUAL: ${mtdApts.length} citas | $${mtdRevenue.toFixed(0)} ingresos hasta hoy (día ${now.getDate()})
...
${topClientsTxt}${clientNotesSummary}${memBlock}
DATOS EN STORAGE:
...
- CLIENTE FRECUENTE: Cuando el usuario mencione a un cliente, usa get_client_profile para traer su historial y notas antes de responder. Usa save_client_note para guardar preferencias cuando las mencionen.
- BÚSQUEDA WEB: Usa web_search para preguntas sobre mercado, competencia, consejos de limpieza, o cualquier dato externo. No respondas sobre datos externos sin buscarlo.
- ANÁLISIS: Cuando el usuario pida análisis, reportes o estadísticas del negocio, usa generate_report con el tipo apropiado.
- DOCUMENTOS: Si el usuario pide "pon en un documento", "hazme un archivo", "descargable", "en un documento", "crea un doc", "ponlo bonito", "dame el formulario", "ponla en un doc", o cualquier variante — genera HTML en un bloque de codigo con lenguaje html. NUNCA digas que no puedes. USA SIEMPRE ESTA ESTRUCTURA:
...
async function executeTool(name, args) {
  switch(name) {
    ...
    case 'github_repo': {
      try{
        const base='https://api.github.com/repos/JMR-Independent/Agenda-LivinGreen/contents/';
        const url=base+(args.path||'').replace(/^\/+/,'');
        const res=await fetch(url,{headers:{'Accept':'application/vnd.github.v3+json','User-Agent':'LivinGreen-Chat'}});
...
// ── Gemini API ─────────────────────────────────────────────────────────────
const _GEMINI_MODELS=['gemini-2.5-pro-preview-05-06','gemini-2.5-pro-exp-03-25','gemini-2.5-pro','gemini-2.5-flash-preview-05-20','gemini-2.5-flash','gemini-2.0-flash-lite','gemini-2.0-flash-exp','gemini-1.5-flash-latest','gemini-1.5-flash-002','gemini-1.5-flash','gemini-1.5-pro'];
